...
export default function InvoiceTable({ invoices, onRefresh }: InvoiceTableProps) {
  ...
  const getInvoiceId = (invoice: Invoice) => invoice.id || invoice._id || "";
      ...
      const checkout = new Razorpay({
        ...
        handler: async (response) => {
          try {
            ...
            alert(
              "Payment was received, but verification failed. Please check the invoice before retrying."
            );
          } finally {
            setPaying(false);
          }
  ...
  const handleStatusChange = async (status: InvoiceStatus) => {
    ...
    try {
      ...
      setSelectedInvoice({ ...selectedInvoice, ...updatedInvoice, status });
      onRefresh?.();
    } catch (error) {
    ...
    <>
          ...
          <table className="w-full min-w-[850px]">
            <thead className="border-b border-border bg-surface-2">
              <tr className="text-left">
                <th className="px-6 py-4 text-sm font-semibold text-text">Invoice ID</th>
                <th className="px-6 py-4 text-sm font-semibold text-text">Customer</th>
                <th className="px-6 py-4 text-sm font-semibold text-text">Total</th>
                <th className="px-6 py-4 text-sm font-semibold text-text">Status</th>
                <th className="px-6 py-4 text-sm font-semibold text-text">Date</th>
                <th className="px-6 py-4 text-sm font-semibold text-text">Action</th>
              </tr>
            </thead>
            <tbody>
              {invoices.length === 0 ? (
                ...
                invoices.map((invoice) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-6 py-5 text-sm font-medium text-text">{getInvoiceId(invoice)}</td>
                      <td className="px-6 py-5 text-sm text-text-secondary">{invoice.customer}</td>
                      <td className="px-6 py-5 font-semibold text-text">
                        ₹{invoice.total.toLocaleString("en-IN")}
                      </td>
                      <td className="px-6 py-5">
                        <span
                          className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold ${getStatusClass(status)}`}
                        >
                      ...
                      <td className="px-6 py-5">
                        <button
                          ...
                          className="flex items-center gap-2 rounded-lg border border-border bg-surface px-4 py-2 text-sm font-medium text-text transition hover:border-primary hover:bg-surface-2"
                        >
      ...
      {selectedInvoice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-border bg-surface p-6 text-text shadow-xl">
            <div className="mb-6 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-text">Invoice Details</h2>
                <p className="mt-1 text-sm text-text-secondary">
                  Invoice ID: {getInvoiceId(selectedInvoice)}
                </p>
              </div>
              <button
                type="button"
                onClick={() => setSelectedInvoice(null)}
                className="rounded-lg p-2 text-text-secondary transition hover:bg-surface-2 hover:text-text"
              >
                <X className="h-6 w-6" />
              </button>
            </div>
            ...
            <div className="mb-6 rounded-xl border border-border bg-surface-2 p-4">
              ...
              <p className="mt-1 text-lg font-semibold text-text">{selectedInvoice.customer}</p>
            </div>
            ...
            {selectedInvoice.status === "Pending" && (
              ...
              >
                ...
                {paying
                  ...
                  : `Pay ₹${selectedInvoice.total.toLocaleString("en-IN")} Online`}
              </button>
            )}

            <div className="mb-6">
              <label className="mb-2 block text-sm font-semibold text-text">Payment Status</label>
              <div className="grid grid-cols-3 gap-3">
                <button
                  ...
                  className={`flex items-center justify-center gap-2 rounded-xl border px-3 py-3 text-sm font-semibold transition ${
                    selectedInvoice.status === "Pending" || !selectedInvoice.status
              ...
              {updating && (
                <p className="mt-2 text-sm text-text-secondary">Updating payment status...</p>
              )}
            </div>
                ...
                <table className="w-full min-w-[500px]">
                  <thead className="bg-surface-2">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-center text-sm font-semibold text-text">Qty</th>
                      <th className="px-4 py-3 text-right text-sm font-semibold text-text">Price</th>
                      <th className="px-4 py-3 text-right text-sm font-semibold text-text">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {selectedInvoice.items?.map((item, index) => (
                      <tr key={index} className="border-t border-border transition-colors hover:bg-surface-2">
                        <td className="px-4 py-4 text-sm text-text">
                          {item.product || item.productId || "Unknown product"}
                        </td>
            ...
            <div className="mt-6 flex items-center justify-between border-t border-border pt-5">
              <span className="text-lg font-semibold text-text-secondary">Invoice Total</span>
            ...
            <div className="mt-6 flex justify-end">
              <button
                type="button"
                onClick={() => setSelectedInvoice(null)}
                className="rounded-xl bg-primary px-5 py-2.5 font-medium text-white transition hover:bg-primary-dark"
              >
                Close
              </button>
            </div>
